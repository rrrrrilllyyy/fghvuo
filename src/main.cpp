...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 5); // Silicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Silicoin Signed Message:\n";
...
{
    // Silicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 318 * COIN;

    // Subsidy is cut in half every 318000 blocks, which will occur a little under every 2 years
    nSubsidy >>= (nHeight / 318000); // Silicoin: 318k blocks in ~<2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Silicoin: retarget every day
static const int64 nTargetSpacing = 3.3 * 60; // Silicoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Silicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Silicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Silicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x13d5b8d448c9c55ad90e8e8a9a9da11e5f09cb111176656cfa98b69631371fc3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1393956264;
        block.nBits    = 0x2000ffff;
        block.nNonce   = 385270803;

        if (fTestNet)
        {
            block.nTime    = 1393956264;
            block.nNonce   = 385271269;
        }
        ...
        if (false)
        {
            ...
            uint256 thash;
            printf("nonce %08X: hash = %s (target = %s)\n", block.nBits, thash.ToString().c_str(), hashTarget.ToString().c_str());
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Silicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SilicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SilicoinMiner : ProcessBlock, block not accepted");
    }
...
void static SilicoinMiner(CWallet *pwallet)
{
    printf("SilicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("silicoin-miner");
    ...
    try { loop {
        ...
        printf("Running SilicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//note: beginning of original loop  

				memcpy(output_data_buffer,pblock,76);
				printf("SENDING BLOCK TO MINER");
				for(int i = 0; i < 76; i++)
					{
						printf("%lu", output_data_buffer[i]);
					}
				serial_transmit_package(79, 4);	
			    printf("Search Block\n");
				
				while(1)
					{
						MilliSleep(1000);
						 printf("Asked for shares\n");
						output_data_buffer[0] = 0;
						output_data_buffer[1] = 0;
						serial_transmit_package(4, 7);
						MilliSleep(1000);
						serial_receive_package();
						if(input_data_buffer[1] == 0x88)
						{
							break;
						}
				...
				printf("Got share\n");
				uint32_t temp = (input_data_buffer[5] << 24) + (input_data_buffer[4] << 16) + (input_data_buffer[3] << 8) + input_data_buffer[2];
				
				pblock->nNonce = temp;
				uint8_t tempbuffer[80];
				memcpy(tempbuffer,pblock,80);
				printf("GOT BLOCK FROM MINER");
				for(int i = 0; i < 80; i++)
					{
						printf("%lu", tempbuffer[i]);
					}
				//memcpy(pblock,&input_data_buffer[3],80);
                // Found a solution
                SetThreadPriority(THREAD_PRIORITY_NORMAL);
                CheckWork(pblock, *pwallet, reservekey);
                SetThreadPriority(THREAD_PRIORITY_LOWEST);
//note: end of original loop                
    ...
    {
        printf("SilicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
   ...
   // for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SilicoinMiner, pwallet));
}
